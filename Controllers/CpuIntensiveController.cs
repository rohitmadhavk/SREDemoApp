...
public class CpuIntensiveController : ControllerBase
{
    ...
    {
        _logger.LogInformation("CPU stress test endpoint called (no-op)");
        return Ok("CPU stress test is disabled in production.");
    }

    [HttpGet("memory-cpu-leak")]
    public ActionResult<string> MemoryCpuLeak()
    {
        _logger.LogInformation("Memory/CPU leak endpoint called (no-op)");
        return Ok("Memory/CPU leak simulation is disabled in production.");
    }
